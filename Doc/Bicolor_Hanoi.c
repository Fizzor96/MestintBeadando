...
int main(int argc, char *argv[])
{
    ...
    void hanoi(int k, disco *primo, disco *secondo, disco *supporto)
    {
        ...
        if (k == 1)
        {
            for (prims = primo; prims->next != NULL; prims = prims->next)
            {
                a = prims;
            }
    ...
    void hanoi_bicolore(int i, int k, disco *primo, disco *secondo, disco *supporto)
    {
        ...
        if (k == i / 2)
        {
            ...
            if (p != NULL && p->color == 'r')
            { //se blu va bene ,altrimenti swappa tutto;
                printf("1");
    ...
    prima = (disco *)malloc(sizeof(disco));
    p = prima;
    prima->dimension = 0;
    prima->color = 'A';
    prima->next = NULL;
    seconda = (disco *)malloc(sizeof(disco));
    s = seconda;
    seconda->dimension = 0;
    seconda->color = 'B';
    seconda->next = NULL;
    appoggio = (disco *)malloc(sizeof(disco));
    appoggio->dimension = 0;
    appoggio->color = 'C';
    appoggio->next = NULL;
    printf("Basi, create\n");
